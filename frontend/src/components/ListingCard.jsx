...
const ListingCard = ({ listing, onUnlock }) => {
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden relative">
      {isPremium && (
        <div className="absolute top-4 right-[-35px] bg-amber-500 text-white text-[10px] font-bold py-1 px-10 rotate-45 shadow-md">
          PREMIUM
        </div>
      )}
      ...
      <div className="p-4">
        ...
        <div className="flex gap-4 mb-6 border-y border-slate-50 py-2">
          <span className="text-xs text-slate-500 flex items-center gap-1"><Bed size={14}/> {listing.beds} Beds</span>
          <span className="text-xs text-slate-500 flex items-center gap-1"><Bath size={14}/> {listing.baths} Baths</span>
        </div>
        ...
        {isUnlocked ? (
          <div className="bg-slate-50 p-3 rounded-lg border border-dashed border-blue-200">
            ...
            <p className="text-sm font-semibold text-blue-600">{listing.contactDetails.phone}</p>
          </div>
        ) : (
          <button 
