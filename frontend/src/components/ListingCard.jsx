...
// Point this path directly to where your frontend initialized firebase "db" object is saved
...
const ListingCard = ({ listing, onUnlock, onReport }) => {
  ...
  // Real-time synchronization state solely for views
  ...
  // Establish Real-Time Listener connection to Firestore for view metrics
  useEffect(() => {
    if (!listing.id) return;

    const docRef = doc(db, 'listings', listing.id);
    
    // Listen for live view data changes instantly
    ...
    // Auto-increment public counter via server-side updates on component init
  ...
  return (
    /* Card Container: Maximized to max-w-2xl on desktop views to sit flush and proportional underneath standard wide desktop search bars */
    <div className="bg-white rounded-none md:rounded-xl border border-slate-200 overflow-hidden w-full max-w-md md:max-w-xl lg:max-w-2xl mx-auto shadow-sm flex flex-col h-full">
      ...
      {/* Analytics Counter Bar - Focused entirely on views */}
      <div className="px-3 pt-3 pb-1 flex items-center justify-between shrink-0">
        <div className="flex items-center gap-1.5 text-slate-700 text-xs font-bold bg-slate-100 px-3 py-1.5 rounded-full shadow-sm">
          <Eye size={15} className="text-slate-800 animate-pulse" /> 
          <span>{liveViews.toLocaleString()} real-time views</span>
        </div>
      </div>

      {/* Technical Details & Info Layout */}
      <div className="px-3 pb-4 space-y-2 flex-grow flex flex-col justify-between">
        <div className="space-y-2">
          ...
          <div className="flex flex-wrap gap-2 text-slate-600 text-xs pt-1">
            ...
            <span className="flex items-center gap-1 bg-slate-100 px-2.5 py-1 rounded-md font-semibold truncate max-w-[220px]">
              <MapPin size={13} className="text-slate-800 shrink-0" /> {listing.address || 'Nigeria'}
            </span>
          </div>
        </div>
