...
const AdminPortal = ({ token }) => {
  ...
  if (error) {
    return (
      <div className="p-4 md:p-8 bg-slate-50 min-h-screen ml-0 md:ml-64 flex flex-col items-center justify-center">
  ...
  return (
    <div className="p-4 md:p-8 bg-slate-50 min-h-screen ml-0 md:ml-64 text-slate-800 transition-all duration-300">
      <div className="mb-6">
              ...
              <div key={item.id} className="bg-slate-50 border border-slate-200 p-4 rounded-lg flex flex-col sm:flex-row sm:items-center justify-between gap-4 text-xs">
                <div className="flex gap-3 items-start w-full">
                  ...
                  <div className="w-full min-w-0">
                    ...
                    <p className="text-slate-500 font-medium truncate">{item.title || "Untitled asset Portfolio"} • ₦{item.price?.toLocaleString()}</p>
                    <p className="text-[11px] text-red-600 mt-1.5 font-medium bg-white px-2 py-1 border border-slate-100 rounded break-words">Reason: {item.reportReason || "Inaccurate item descriptions"}</p>
                  </div>
                </div>
            ...
            {queues.kyc.map(item => (
              <div key={item.id} className="bg-slate-50 border border-slate-200 p-4 rounded-lg flex flex-col sm:flex-row sm:items-center justify-between gap-4 text-xs">
                <div className="flex gap-3 items-start w-full">
                  ...
                  <div className="w-full min-w-0">
                    <div className="font-black text-slate-900 mb-0.5 truncate">Agent KYC: {item.fullName || "Broker Identity"}</div>
                    <p className="text-slate-500 font-medium truncate">ID Type: {item.idType || "NIN/Passport"} • Doc #{item.idNumber || "XXXX"}</p>
                    {item.documentUrl && (
              ...
              <div key={item.id} className="bg-slate-50 border border-slate-200 p-4 rounded-lg flex flex-col sm:flex-row sm:items-center justify-between gap-4 text-xs">
                <div className="flex gap-3 items-start w-full">
                  ...
                  <div className="w-full min-w-0">
                    ...
                    <p className="text-slate-600 italic font-medium bg-white px-2 py-1 border border-slate-100 rounded mt-1 break-words">"{item.reviewText || 'No comment content data body.'}"</p>
                  </div>
                </div>
                <div className="flex gap-2 w-full sm:w-auto shrink-0 justify-end">
