    ...
    <div className="p-4 md:p-8 bg-slate-50 min-h-screen w-full md:pl-72 flex flex-col items-center">
      <div className="w-full max-w-5xl">
        <div className="mb-6 self-start w-full text-left">
          ...
          <p className="text-xs md:text-sm text-slate-500">Sort, edit, and keep track of active placements.</p>
        </div>
            ...
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-slate-100 text-sm">
                {myListings.map((property) => (
                  <tr key={property.id} className="hover:bg-slate-50/40 transition">
                    <td className="p-4">
                    ...
                    <td className="p-4 font-semibold text-slate-700">₦{property.price?.toLocaleString() || '0'}</td>
                    <td className="p-4">
                      <span className={`inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-xs font-medium ${property.status === 'active' ? 'bg-emerald-100 text-emerald-800' : 'bg-amber-100 text-amber-800'}`}>
                        {property.status || 'pending'}
                      </span>
                    </td>
                    <td className="p-4 text-center text-slate-500 font-medium">
                      <span className="inline-flex items-center gap-1"><Eye size={14} /> {property.views || 0}</span>
                    </td>
                    <td className="p-4 text-right space-x-1">
                      ...
                      <button onClick={() => handleDelete(property.id)} className="p-2 text-slate-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition"><Trash2 size={16} /></button>
                    </td>
                  </tr>
                ))}
              </tbody>
          ...
          {myListings.map((property) => (
            <div key={property.id} className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm space-y-3">
              ...
              <div className="flex justify-between items-center pt-2 border-t border-slate-100 text-sm">
                <span className="font-bold text-slate-700">₦{property.price?.toLocaleString() || '0'}</span>
              ...
              <div className="flex justify-between items-center pt-2 border-t border-slate-100">
                <span className="text-xs text-slate-500 flex items-center gap-1"><Eye size={14} /> {property.views || 0} views</span>
        ...
        {myListings.length === 0 && (
          <div className="p-12 text-center text-slate-400 bg-white rounded-xl border border-slate-200 italic w-full shadow-sm mt-4">
